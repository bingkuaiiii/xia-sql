      ...
      while(true) {
         ...
         do {
            if (!var42.hasNext()) {
               ...
               while(var42.hasNext()) {
                  Request_md5 i = (Request_md5)var42.next();
               ...
               while(var54.hasNext()) {
                  IParameter para = (IParameter)var54.next();
                  int switch_para = false;
                  if (para.getType() == 6) {
                     ++json_count;
                  }
                  ...
                  payloads.add("'");
                  payloads.add("''");
                  if (para.getType() == 0 || para.getType() == 1 || para.getType() == 6 || para.getType() == this.is_cookie) {
                     ...
                     for(Iterator var57 = payloads.iterator(); var57.hasNext(); this.log2.add(new LogEntry(this.conut, toolFlag, this.callbacks.saveBuffersToTempFiles(requestResponse), this.helpers.analyzeRequest(requestResponse).getUrl(), key, value + payload, change_sign, temp_data, time_2 - time_1, "end", this.helpers.analyzeResponse(requestResponse.getResponse()).getStatusCode()))) {
                        ...
                        if (para.getType() != 6) {
                           ...
                           IParameter newPara = this.helpers.buildParameter(key, value + payload, para.getType());
                           ...
                           if (is_add == 1) {
                              ...
                              for(int i = 0; i < request_data_temp.length; ++i) {
                                 if (i != json_count) {
                                    request_data_body = request_data_body + "\"" + request_data_temp[i] + ",";
                                 } else {
